...
import tracks from '../data/cosmic-radio-tracks.json';
...
export function CosmicRadio() {
  ...
  useEffect(() => {
    try {
      const saved = window.localStorage.getItem('harry-media-cosmic-radio-volume'), next = saved === null ? 0.2 : Number(saved);
  ...
  const changeVolume = (next: number) => {
    ...
    try { window.localStorage.setItem('harry-media-cosmic-radio-volume', String(next)); } catch {}
  };
  const pause = () => { request.current += 1; wanted.current = false; audioRef.current?.pause(); setStatus('paused'); };
  const play = (next = indexRef.current) => {
    ...
    const src = new URL(tracks[next].src, MUSIC_ROOT).href;
  ...
  const skip = (direction: number) => play((indexRef.current + direction + tracks.length) % tracks.length);

  return (
    <aside className={`cosmic-radio${status === 'playing' ? ' is-playing' : ''}`} aria-label="Harry's cosmic radio">
      ...
      <div className="cosmic-radio-top">
        <button type="button" className="cosmic-radio-pet" onClick={() => (active ? pause() : play())} aria-label={active ? 'Pause radio' : 'Play radio'}><img src={pet} alt="" width="64" height="64" /></button>
