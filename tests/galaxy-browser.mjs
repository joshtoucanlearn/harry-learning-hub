...
const fallback=await browser.newPage();await fallback.addInitScript(()=>{const original=HTMLCanvasElement.prototype.getContext;HTMLCanvasElement.prototype.getContext=function(type,...args){return type==='webgl2'?null:original.call(this,type,...args)}});await fallback.goto(url);await fallback.getByRole('heading',{name:'All yours, Harry.'}).waitFor();const background=await fallback.locator('.galaxy-backdrop').evaluate(el=>getComputedStyle(el).backgroundImage);assert.match(background,/galaxy-still/);const fallbackAsset=background.match(/url\(["']?(.*?)["']?\)/)[1];assert.equal((await fallback.request.get(fallbackAsset)).ok(),true);assert.equal(await fallback.locator('.sky-toggle').count(),0);
